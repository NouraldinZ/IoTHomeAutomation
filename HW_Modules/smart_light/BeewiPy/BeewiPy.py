...
class BeewiSmartBulb:
    ...
    def increaseBrightness():
        ...
        if self.brightness == 9:
            print("Cannot increase brightness.")
        else:
            newBrightness = self.brightness + 1
            self.__writeSettings(BeewiSmartBulb.SET_BRIGHTNESS[newBrightness])

    def decreaseBrightness():
        self.__readSettings()
        if self.brightness == 0:
            print("Cannot decrease brightness.")
        else:
            newBrightness = self.brightness - 1
            self.__writeSettings(BeewiSmartBulb.SET_BRIGHTNESS[newBrightness])
    ...
    def increaseTemperature():
        ...
        if self.temperature == 9:
            print("Cannot increase temperature.")
        else:
            newTemperature = self.temperature + 1
            self.__writeSettings(BeewiSmartBulb.SET_TEMPERATURE[newTemperature])

    def decreaseTemperature():
        self.__readSettings()
        if self.temperature == 0:
            print("Cannot decrease temperature.")
        else:
            newTemperature = self.temperature - 1
            self.__writeSettings(BeewiSmartBulb.SET_TEMPERATURE[newTemperature])
